...
#preprocess_data = function(filepath, region, analysis_num, chunk_size, num_genes = 2500, gene_num_tolerance = 100, var_thresh_start = 2, max_var_thresh = 4, customGeneList = NA, return.object = FALSE){
  ...
  if(length(meth_files)>0){
    ...
    for(i in 1:length(meth_files)){
      ...
      met.cell.data$dataset = as.character(met.cell.data$dataset)
      met.cell.data$barcode = as.character(met.cell.data$barcode)
      rhdf5::h5createFile(meth_files[i])
      rhdf5::h5createGroup(meth_files[i], "matrix")
      rhdf5::h5write(colnames(current_matrix), file=meth_files[i], name="matrix/barcodes")
      rhdf5::h5createGroup(meth_files[i], file.path("matrix", "features"))
      rhdf5::h5write(rownames(current_matrix), file=meth_files[i], name="matrix/features/name")
      rhdf5::h5write(dim(current_matrix), file=meth_files[i], name="matrix/shape")
      rhdf5::h5createDataset(meth_files[i],"matrix/data",dims=3,storage.mode="double", chunk = 1)
      rhdf5::h5write(1:3, file=meth_files[i], name="matrix/data", index = list(1:3))
      rhdf5::h5createDataset(meth_files[i],"matrix/indices",dims=3,storage.mode="integer", chunk = 1)
      rhdf5::h5write(1:3, file=meth_files[i], name="matrix/indices",index = list(1:3)) # already zero-indexed.
      rhdf5::h5createDataset(meth_files[i],"matrix/indptr",dims=3,storage.mode="integer", chunk = 1)
      rhdf5::h5write(1:3, file=meth_files[i], name="matrix/indptr",index = list(1:3))
      rhdf5::h5createDataset(meth_files[i],"norm.data",dims=3,storage.mode="integer", chunk = 1)
      rhdf5::h5write(1:3, file=meth_files[i], name="norm.data",index = list(1:3))
      rhdf5::h5createDataset(meth_files[i],"scale.data",dims=c(nrow(current_matrix),ncol(current_matrix)),storage.mode="double", chunk = c(nrow(current_matrix),chunk_size))
      rhdf5::h5write(current_matrix, file=meth_files[i], name="scale.data",index = list(NULL, 1:ncol(current_matrix)))
      rhdf5::h5write(met.cell.data, file=meth_files[i], name="cell.data")
      rhdf5::h5closeAll()
    }
  }
