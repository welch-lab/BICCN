...
generate_slice_gifs = function(
  ...
  axes_follow = which(colnames(coords) != axis)
  if(length(axes_follow) == ncol(coords)){
    message("Invalid axis selected. Possible axes provided here:")
  ...
  if(mat.use == "assignment"){
    ...
    #TODO
    if(!is.null(cell.types.use)){
      cell.types.use = intersect(cell.types.use, levels(assignments))
    } else {
      cell.types.use = levels(assignments)
    }
    cell.types.use = cell.types.use[cell.types.use != ""]
    assignments = assignments[assignments %in% cell.types.use]
    levels(assignments) = sub("/",".",sub(" ", "_", cell.types.use))
    shared_samples = intersect(names(assignments), rownames(coords))
    assignments = assignments[shared_samples]
    coords = coords[shared_samples,]

    plotting_df = as.data.frame(cbind(coords, assignments))

    slice_plot = ggplot(plotting_df, aes_string(x = colnames(plotting_df)[axes_follow[1]],
                                                y = colnames(plotting_df)[axes_follow[2]],
                                                fill = "assignments")) + #what is the column named
      geom_tile() +
      coord_fixed(ratio = 1) +
      viridis::scale_fill_viridis() +
      ggtitle(paste0("Distribution of cell types")) +
      theme(legend.title = element_blank(),
            text = element_text(size = 8),
            axis.text = element_text(size = 5)) +
      transition_time(parse(colnames(plotting_df)[axes_follow[3]])) + #unsure if works
      labs(subtitle = paste0("In ", colnames(plotting_df)[axes_follow[3]], " slice {as.integer(frame_time)}"))


      animate(slice_plot, height = 800, width = 800, fps = 30, duration = 10,
               end_pause = 60, res = 100, render = magick_renderer())
      anim_save(paste0(filepath,"/",  region,"/", region,"_Deconvolution_Output/",spatial.data.name,"/plots/cell_assignments_",axis,"_slice.gif"))
    ...
    if(use.cell.types){
      ...
      colnames(loadings) = sub("/",".",sub(" ", "_", cell.types.use))
      shared_samples = intersect(rownames(loadings), rownames(coords))
      loadings = loadings[shared_samples,]
      coords = coords[shared_samples,]
      ...
      for(i in 1:ncol(loadings)){
        slice_plot = ggplot(plotting_df, aes_string(x = colnames(plotting_df)[axes_follow[1]],
          ...
          theme(legend.title = element_blank(),
                text = element_text(size = 8),
                axis.text = element_text(size = 5)) +
          transition_time(parse(text = axis)) + #unsure if works
          ease_aes('linear')
          #labs(subtitle = paste0("In ", axis, " slice {as.integer(frame_time)}"))


          animate(slice_plot, height = 800, width = 800, fps = 30, duration = 10,
                   end_pause = 60, res = 100)
          anim_save(paste0(filepath,"/",  region,"/", region,"_Deconvolution_Output/",spatial.data.name,"/plots/",
                           colnames(loadings)[i],"_",axis,"_slice.gif"),
                    animation = slice_plot)
      }
